    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
        @foreach ($cards as $card)
            <div class="{{ $card['bg'] }} text-white p-4 sm:p-6 rounded-2xl shadow hover:shadow-xl transition flex flex-col justify-between">
                ...
                <a href="{{ $card['route'] }}" 
                   class="block sm:inline-block w-full sm:w-auto text-center bg-white {{ $card['text'] }} px-3 py-2 rounded-lg font-semibold text-xs sm:text-sm hover:bg-gray-100 transition">
                   Manage
                </a>
            </div>
        @endforeach
