...
<div class="p-6 bg-gray-50 min-h-screen">
    ...
    <div class="mb-6">
        ...
        <p class="text-gray-600 mt-1">Senang melihatmu kembali! Yuk, lanjutkan membaca hari ini 📖</p>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div class="bg-white p-4 rounded-2xl shadow">
            <h2 class="text-gray-500 text-sm">Buku Dipinjam</h2>
            <p class="text-3xl font-semibold text-indigo-600">3</p>
        </div>
        <div class="bg-white p-4 rounded-2xl shadow">
            <h2 class="text-gray-500 text-sm">Tenggat Terdekat</h2>
            <p class="text-xl font-semibold text-red-500">22 Okt 2025</p>
        </div>
        <div class="bg-white p-4 rounded-2xl shadow">
            <h2 class="text-gray-500 text-sm">Total Riwayat</h2>
            <p class="text-3xl font-semibold text-green-600">15</p>
        </div>
        <div class="bg-white p-4 rounded-2xl shadow">
            <h2 class="text-gray-500 text-sm">Kategori Favorit</h2>
            <p class="text-lg font-semibold text-blue-600">Teknologi</p>
        </div>
    </div>

    <div class="mb-8">
        <h2 class="text-xl font-bold text-gray-800 mb-4">📚 Rekomendasi Buku</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            @foreach ($recommendedBooks as $book)
                <div class="bg-white p-4 rounded-2xl shadow hover:shadow-md transition">
                    ...
                    <a href="{{ route('user.books.detail', $book->id) }}" class="text-sm text-blue-500 hover:underline">Lihat Detail →</a>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="mb-8">
        <h2 class="text-xl font-bold text-gray-800 mb-4">🕓 Riwayat Terbaru</h2>
        <table class="w-full bg-white rounded-2xl shadow overflow-hidden">
            <thead class="bg-gray-100 text-gray-700">
                <tr>
                    <th class="text-left p-3">Judul Buku</th>
                    ...
                    <tr class="border-b">
                        ...
                        <td class="p-3">
                            <span class="px-2 py-1 text-sm rounded-xl 
                                {{ $history->status == 'dipinjam' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700' }}">
