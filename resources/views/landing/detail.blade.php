                    ...
                    @elseif(Auth::user()->role === 'admin')
                        <div class="flex gap-3">
                            <a href="{{ route('books.edit', $book->id) }}" 
                               class="flex-1 text-center bg-yellow-400 hover:bg-yellow-500 text-white font-semibold py-2 rounded-lg">✏️ Edit</a>
                            <form action="{{ route('books.destroy', $book->id) }}" method="POST" class="flex-1">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
                                    class="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-2 rounded-lg">🗑 Hapus</button>
                            </form>
                        </div>
                    @endif
                @else
